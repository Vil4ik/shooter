...
class Player(GameSprite):
    ...
    def update(self):
        ...
        elif keys[K_d] and self.rect.x < 635:
            self.rect.x += self.speed

    def fire(self):
        bullet = Bullet('bullet.png', self.rect.centerx - 5, self.rect.y, 10, 20, 20)
...
score_font = FontClass('Счёт:', 5, 10)
missed_font = FontClass('Пропущено:', 5, 30)

enemys = sprite.Group()
for i in range(5):
    enemys.add(
        Enemy('ufooo.png', randint(0, 635), -70, 65, 65, randint(1, 3))  
    )
hero = Player('oo.png', 317.5, 420, 65, 65, 5)
...
win = FontClass('Победа', 270, 235, (0, 255, 0), 60)
...
while game:
    ...
    if not finish:
        ...
        spritecollides =  sprite.spritecollide(hero, enemys, False)
